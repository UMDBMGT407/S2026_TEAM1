...
# IMPORTS
# =========================
from flask import Flask, request, render_template, redirect, url_for, abort, jsonify, Response, flash
...
# =========================
import anthropic
from ai_assistant_module import get_inventory_snapshot, build_system_prompt, call_ai
...
@app.route('/sl-ai-assistant')
@login_required
@role_required('ShiftLead')
def sl_ai_assistant_page():
    return render_template('sl_ai_assistant.html')
...
def ai_chat():
    ...
    provider     = (data.get('provider') or 'groq').lower().strip()
    ...
    cur = mysql.connection.cursor()
    try:
        snapshot = get_inventory_snapshot(cur)
    except Exception as db_err:
        cur.close()
        return jsonify({'error': f'Database error: {str(db_err)}'}), 500
    cur.close()
    system_prompt = build_system_prompt(current_user.role, snapshot)
    messages = []
    for turn in raw_history[-20:]:
        role    = turn.get('role', '')
        content_t = (turn.get('content') or '').strip()
        if role in ('user', 'assistant') and content_t:
            messages.append({'role': role, 'content': content_t})
    ...
    try:
        reply = call_ai(provider, api_key, system_prompt, messages)
    except Exception as api_err:
        err = str(api_err).lower()
        if 'auth' in err or 'invalid' in err or 'api_key' in err:
            return jsonify({'error': f'Invalid {provider.title()} API key.'}), 500
        if 'rate' in err or 'limit' in err:
            return jsonify({'error': 'Rate limit reached. Try again shortly.'}), 429
        return jsonify({'error': f'AI error: {str(api_err)}'}), 500
